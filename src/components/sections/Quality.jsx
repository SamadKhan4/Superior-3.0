        ...
        <div className="lg:col-span-5">
          ...
          <FadeUp className="mt-8 max-w-md">
            <p className="text-[16px] leading-relaxed text-ink/70">
              Quality on a mesh order is decided long before dispatch — in the wire we accept, the
              machine settings we hold, and the checks we run between batches. Each stage below is a
              fixed part of how an order moves through our plant.
            </p>
          </FadeUp>
              ...
              <li key={entry.number} className="relative">
                ...
                >
                  <div className="flex items-baseline gap-3 sm:gap-5">
                    <span
                      className={`font-mono text-[11px] transition-colors duration-200 ${active === i ? 'text-molten' : 'text-ink/35'}`}
                    >
                      {entry.number}
                    </span>
                    <span
                      className={`font-display text-[clamp(1.35rem,2.6vw,2rem)] font-medium uppercase tracking-tight transition-[color,transform] duration-300 ease-weld ${active === i ? 'translate-x-1 text-ink' : 'text-ink/45 group-hover:text-ink/75'}`}
                    >
                      {entry.title}
                    </span>
                  </div>
