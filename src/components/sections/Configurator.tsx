...
{ key: 'coating', label: 'Coating', options: ['GI', 'Powder Coated'] }] as
const;

type ConfigKey = (typeof CONTROLS)[number]['key'];

const WIRE_TONE: Record<string, string> = {
  MS: '#7C858B',
  GI: '#ABB2B7',
...
export function Configurator() {
  ...
  const stroke =
  config.coating === 'Powder Coated' ? '#F47B20' : WIRE_TONE[config.material];
        ...
        <div className="lg:col-span-5">
          ...
          <FadeUp className="mt-8 max-w-md">
            <p className="text-[15px] leading-relaxed text-chalk/65">
              Adjust the variables the way a project drawing would. Material,
              aperture, form and coating all change what we set up on the line —
              dimensions are engineered to your specification.
            </p>
          </FadeUp>
          ...
          <div className="mt-10 space-y-7">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  ...
                  return (
                    <button
                      ...
                      className={`border px-4 py-2.5 font-mono text-[11px] uppercase tracking-[0.16em] transition-colors duration-200 ${
                      selected ?
                      'border-molten bg-molten text-ink' :
                      'border-metal/25 text-chalk/70 hover:border-metal/50 hover:text-chalk'}`
                      }>
            ...
            <div>
              ...
              <p className="mt-3 font-display text-[22px] text-chalk">
                Custom — to drawing
              </p>
            </div>
          </div>
          ...
          <div className="relative aspect-square w-full overflow-hidden border border-metal/15 bg-ink">
            <svg
              ...
              <motion.g
                ...
                Array.from({ length: Math.ceil(400 / cellH) + 1 }).map((_, r) =>
                <motion.circle
                  key={`p${c}-${r}`}
                  cx={c * cellW}
                  cy={r * cellH}
