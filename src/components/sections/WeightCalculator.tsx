...
const FIELDS: {key: keyof Fields;label: string;unit: string;}[] = [
...
export function WeightCalculator() {
  ...
  const result = useMemo(() => {
    const d = parseFloat(fields.diameter) / 1000;
    const o = parseFloat(fields.opening) / 1000;
        ...
        <div className="lg:col-span-5 lg:pt-6">
          ...
          <FadeUp className="mt-8 max-w-md">
            <p className="text-[16px] leading-relaxed text-ink/70">
              Quickly estimate wire mesh weight based on your dimensional
              requirements — useful before you finalise quantities, freight or
              a tender submission.
            </p>
          </FadeUp>
          ...
          <div className="relative border border-ink/12 bg-chalk p-6 sm:p-9">
            ...
            <div className="mt-10 border-t border-ink/12 pt-8">
              ...
              <p className="mt-4 font-mono text-[11px] uppercase tracking-[0.18em] text-ink/50">
                {result ?
                `${result.perSheet.toFixed(2)} kg per sheet · ${fields.quantity} nos · density 7850 kg/m³` :
