...
function Hero({ ready }) {
  ...
  const line = (i) => ({
    ...
    animate: show ? (reduce ? { opacity: 1 } : { y: 0 }) : reduce ? { opacity: 0 } : { y: '100%' },
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        animate={show ? (reduce ? { opacity: 1 } : { clipPath: 'inset(0% 0% 0% 0%)' }) : void 0}
      ...
      >
        <motion.img
          ...
          style={reduce ? void 0 : { scale, y: imageY }}
        />
      ...
      >
        ...
        <h1 className="font-display font-semibold uppercase leading-[0.94] tracking-tightest text-chalk text-[clamp(3rem,9.2vw,6.9rem)]">
          ...
          <span className="sr-only">Wire Mesh &amp; Fencing Solutions Engineered for Strength</span>
        </h1>
        <motion.p
        ...
        >
          Precision-manufactured wire mesh, reinforcement and fencing solutions engineered for
          infrastructure, construction, security and demanding industrial applications.
        </motion.p>
        <motion.div
        ...
        <motion.dl
          ...
          initial={{ opacity: 0 }}
          animate={show ? { opacity: 1 } : void 0}
          transition={{ duration: 0.7, ease: EASE, delay: 1 }}
        >
          {heroStats.map((stat) => (
            <div key={stat.index} className="flex items-start gap-4">
              <span className="mt-1 font-mono text-[10px] text-molten">{stat.index}</span>
              <div>
