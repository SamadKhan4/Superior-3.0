...
export function Navigation() {
  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-[background-color,border-color,backdrop-filter] duration-300 ease-weld ${
      scrolled || megaOpen ?
      'border-b border-metal/15 bg-ink/85 backdrop-blur-md' :
      ...
      <div
        ...
        <a href="#top" className="flex items-baseline gap-3">
          <span
            className={`font-display font-semibold tracking-tightest text-chalk transition-[font-size] duration-300 ease-weld ${
            scrolled ? 'text-[20px]' : 'text-[24px]'}`
            }>
            
            SWPL
          </span>
          <span className="hidden font-mono text-[9px] uppercase tracking-[0.3em] text-steel sm:block">
            Superior Weldmesh
          </span>
        </a>
          ...
          {navLinks.map((link) => {
            ...
            return (
              <div
                ...
                <a
                  href={link.href}
                  aria-haspopup={isProducts || undefined}
                  aria-expanded={isProducts ? megaOpen : undefined}
                  className={`group relative block font-mono text-[11px] uppercase tracking-[0.18em] transition-colors duration-200 ${
                  isProducts && megaOpen ?
                  'text-molten' :
